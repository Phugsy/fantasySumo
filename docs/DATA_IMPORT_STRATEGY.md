...
7. Add a protected scheduled production trigger that locks the eligible basho
   ...
   Allow day-0 lock catch-up, then import every day after stored `currentDay`
   through the calculated day so missed invocations recover in order. Move
   upcoming or locked bashos to active with day 1 and complete the basho with
   day 15. Keep deterministic demo bashos outside this path.
